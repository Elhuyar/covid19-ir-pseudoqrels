...
def process_results_light(indri_results,index):
    output=[]
    count=0
    ...
    for int_document_id, score in indri_results:
        ...
        doc ={"doc_id":ext_document_id,  "indri_score":indri_score, "rank":int_document_id}
        output.append(doc)
...
def main(args):
    ...
    index_doc_path=os.path.join(index_root,'BildumaECIR')  # not filtered 
    ...
    prf_query_env = pyndri.PRFQueryEnvironment(lm_query_env, fb_docs=45, fb_terms=25)# best config without filter words:  fb_docs=35, fb_terms=25
    ...
    for index, row in queries_df.iterrows(): 
        ...
        try:
            ...
            if krovetz_stem:
                tokenized_query=" ".join([pyndri.krovetz_stem(t.encode("latin-1",'ignore').decode('latin-1')) for t in tokens if not t in stopwords]).replace('--',' ').replace(' -',' ')
                tokenized_query2=" ".join([pyndri.krovetz_stem(t.encode("latin-1",'ignore').decode('latin-1')) for t in tokens if not t in stopwords]).replace('--',' ').replace(' -',' ')
            ...
            #results = lm_query_env.query(tokenized_query, results_requested=maxdocs)
            ...
            #docs = process_results_light(results,index_doc)
