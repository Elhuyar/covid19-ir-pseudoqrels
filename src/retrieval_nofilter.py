...
def main(args):
    ...
    metadata_doc=pd.read_csv(os.path.join(metadata_path,metadata),dtype={"mag_id":str,"who_covidence_id":str,"arxiv_id":str},low_memory=False)
    ...
    metadata_pas=pd.read_csv(os.path.join(metadata_path,passage_metadata),low_memory=False)
    ...
    index_doc_path=os.path.join(index_root,'BildumaTRECAbsIndex_round5_Notfiltered_bis') 
    ...
    prf_query_env = pyndri.PRFQueryEnvironment(lm_query_env,fb_docs=35, fb_terms=25) #fb_docs=20, fb_terms=10)#
